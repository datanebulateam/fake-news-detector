...
msg_feedback = '*Deixe o seu feedback:* Essa predição foi útil?'
...
def process_message_step(message):
    ...
    if validators.url(noticia):
        ...
        callback = bot.reply_to(message, outcome, parse_mode= 'Markdown')     
...
def verify_message(message):
    if message.text == 'Sim':
        ...
        callback = bot.send_message(message.chat.id, msg_feedback, parse_mode= 'Markdown')
        bot.register_next_step_handler(callback, get_feedback)
